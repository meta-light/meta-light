...
| Field      | Value                                           |
| ---------- | ----------------------------------------------- |
| Identifier | `openclaw-skills-x-algorithm`                                |
| Version    | 1.0.3                                     |
| Author     | openclaw                                 |
| Category   | social                                    |
| Installs   | 9                                |
...
**GitHub:** [openclaw/skills](https://github.com/openclaw/skills) — ⭐ 1086 | Forks: 360
...
x-algorithm provides a compact, production-ready rule set and tooling for AI agents to maximize X (Twitter) engagement while minimizing reach death. It encodes platform signals (timing, cadence, thread architecture, reply loops, hashtag/mention hygiene, media optimization) into prompt templates and vibe-coding modules so Cursor, Claude, ChatGPT, and Copilot can generate algorithm-friendly posts, replies, and experiments. Features include posting cadence plans, thread and headline patterns, safety/policy heuristics to avoid penalties, A/B testing scaffolds, and metric-tracking recommendations (impressions, CTR, engagement rate). Use cases: autonomous social agents, community managers, growth teams, and researchers automating campaign tests. Core advantages are reproducible engagement lift, reduced risk of suppressions, seamless integration into LLM workflows, and easy customization of voice/persona through vibe-coding.
...
```bash
npx -y @lobehub/market-cli skills install openclaw-skills-x-algorithm
```
...
npx -y @lobehub/market-cli skills install openclaw-skills-x-algorithm --agent open-claw

# Claude Code (installs to ./.claude/skills/)
npx -y @lobehub/market-cli skills install openclaw-skills-x-algorithm --agent claude-code

# Codex (installs to ./.agents/skills/)
npx -y @lobehub/market-cli skills install openclaw-skills-x-algorithm --agent codex

# Cursor (installs to ./.cursor/skills/)
npx -y @lobehub/market-cli skills install openclaw-skills-x-algorithm --agent cursor
```

Install a specific version:

```bash
npx -y @lobehub/market-cli skills install openclaw-skills-x-algorithm --version 1.0.3
```
...
```bash
npx -y @lobehub/market-cli skills rate openclaw-skills-x-algorithm --score <1-5>
```
...
```bash
npx -y @lobehub/market-cli skills comment openclaw-skills-x-algorithm -c "Your feedback here" --rating <1-5>
```
...
```bash
npx -y @lobehub/market-cli skills comments openclaw-skills-x-algorithm
```
