...
const GetHashlist = () => {
    ...
    const getAssetsByGroup = async () => {
      const response = await fetch(url, {
        ...
        body: JSON.stringify({
          ...
          params: { groupKey: 'collection', groupValue: dewicats, page: 1, limit: 1000 }
        }),
      });
      const { result } = await response.json();
      const owners = result.items.map(item => item.ownership.owner);
      const hashes = result.items.map(item => item.id);
      fs.writeFile('owners.json', JSON.stringify(owners, null, 2), (err) => {
        if (err) throw err;
        console.log('Owners have been saved to owners.json!');
      });
      fs.writeFile('hashes.json', JSON.stringify(hashes, null, 2), (err) => {
        if (err) throw err;
        console.log('Hashes have been saved to hashes.json!');
      });
    };
    getAssetsByGroup();
  }, []);
  ...
  return (
    <div>
      <h1>Assets By Group</h1>
      <p>Data is being fetched and saved to files...</p>
    </div>
  );
};
