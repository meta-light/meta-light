/**
 * Acid-test fixture: eeefano's transcription of Corona's "The Rhythm Of The
 * Night" — a real community song with everything that stresses the timeline:
 * register() helpers, const definitions, one giant stack(...) (with the
 * `stack` keyword on its own line), per-voice `"<...>".pickRestart([...])`
 * timelines summing to 54 cycles, per-voice .color(), and a trailing
 * .cpm(128/4) that rescales the whole song by a rational factor.
 */
export const CORONA = `// "The Rhythm Of The Night" - Work In Progress
// song @by Corona
// script @by eeefano
